...
export default function BountyDetailPage() {
  ...
  const handleAccept = async () => {
    ...
    if (!user) {
      router.push("/login?redirect=/worker/browse");
      return;
    }
    ...
    if (updateError) {
      ...
      router.push(`/worker/my-tasks/${id}`);
    }
  };
  ...
  return (
    <div>
      <Link href="/worker/browse" style={{ display: "inline-flex", alignItems: "center", gap: 4, marginBottom: 24, fontSize: 12, color: "var(--oc-text-muted)", textDecoration: "none", fontFamily: "var(--oc-font)" }}>
        ← Back to Browse
      </Link>
