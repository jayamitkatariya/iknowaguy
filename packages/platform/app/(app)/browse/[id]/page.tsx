...
export default function BountyDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchBounty = async () => {
      ...
      try {
        ...
        if (!cancelled) setBounty(response.data);
      } catch (err: any) {
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      const humanId = authData?.user?.id;
      if (!humanId) throw new Error("Not logged in");
      await apiFetch(`/api/bounties/${id}/accept`, {
        method: 'POST',
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
          <div>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {bounty.category?.name && <span className="oc-badge oc-badge-gray">{bounty.category.name}</span>}
              <span className={`oc-badge ${bounty.status === "open" ? "oc-badge-green" : "oc-badge-amber"}`}>{bounty.status?.replace(/_/g, " ")}</span>
            </div>
          </div>
