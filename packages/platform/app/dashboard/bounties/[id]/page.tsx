...
export default function BountyDetailPage() {
  ...
  const fetchBounty = async () => {
    try {
      const res = await apiFetch(`/api/bounties/${id}`);
      setBounty(res.data);
  ...
  const handleReview = async (decision: "approved" | "rejected") => {
    try {
      ...
      await apiFetch(`/api/bounties/${sub.id}/review`, {
        method: "POST",
  ...
  const statusColors: Record<string, string> = {
    ...
    revision_requested: "oc-badge-red",
  ...
  return (
    <div style={{ padding: "2rem", maxWidth: "900px", margin: "0 auto" }}>
      ...
      <header style={{ marginBottom: "2rem" }}>
        ...
        <p style={{ color: "var(--oc-text-muted)" }}>{bounty.category?.name && `Category: ${bounty.category.name}`}</p>
      </header>
