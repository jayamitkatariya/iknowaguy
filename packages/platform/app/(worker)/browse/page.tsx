...
export default function BrowsePage() {
  ...
  useEffect(() => {
    ...
    const fetchBounties = async () => {
      setLoading(true);
      try {
        const response = await apiFetch('/api/bounties?status=open&scope=all');
        if (!cancelled) {
    ...
    fetchBounties();
    apiFetch('/api/categories').then((response) => {
  ...
  const filtered = bounties.filter((b) => {
    ...
    const matchCat = category === "all" || b.category?.name === category;
