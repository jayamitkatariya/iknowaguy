...
function startServer(name, command, args, port, color) {
  const server = {
    name,
    port,
    process: null,
...
async function main() {
  ...
  startServer('MCP HTTP', 'npx', ['tsx', 'src/index-http.ts'], 3001, colors.magenta);

  // Wait a moment for MCP server to initialize
  await new Promise(resolve => setTimeout(resolve, 1500));

  // Start REST API (port 3000)
  startServer('REST API', 'npx', ['tsx', 'src/index.ts'], 3000, colors.blue);

  // Start Worker App (port 3002)
  startServer('Worker App', 'npx', ['next', 'dev', '-p', '3002'], 3002, colors.green);

  // Start Admin Dashboard (port 3003)
  startServer('Admin Dashboard', 'npx', ['next', 'dev', '-p', '3003'], 3003, colors.yellow);
